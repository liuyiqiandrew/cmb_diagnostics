...
- **Debug PNGs.** V2's `SOPlkTF.__tf_ee` wrote `debug_dust_fit_*.png` + `debug_tf_fit_*.png` per ℓ-bin into CWD. The new estimator never touches the filesystem; per-bin fit data (`r`, `dust_amp`, `chi2_tf`) is captured in `FitResult.diagnostics`. When `cfg.advanced.write_diagnostic_plots=True`, `Pipeline.run()` calls `reports.tf.plot_diagnostics` to write `{out_dir}/diagnostics/{result.name}_diagnostics.png` summarizing those quantities per ℓ-bin. The one-figure-per-bin scatter plots of the legacy code are not reproduced; the underlying diagnostic arrays are saved in the `.npz` if you want to reconstruct them.
